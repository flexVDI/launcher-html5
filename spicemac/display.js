...
SpiceDisplayConn.prototype.hook_events = function()
{
    if (this.primary_surface !== undefined)
    {
        ...
        input.sc = this.parent;
        input.addEventListener('compositionend', handle_compositionend);
        addEventListener('keypress', handle_keypress);
        addEventListener('keydown', handle_keydown);
        addEventListener('keyup', handle_keyup);
        input.focus();

        canvas.onfocus=function(){input.focus();};
    }
}
