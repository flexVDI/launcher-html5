...
function send_char_tilde_key(ctx, charcode, codelist)
{
    var codelistLength = codelist.length;
    ...
    if (codelist[0] == 1) {
        shiftMod = true; 
    } else if (codelist[0] == 2) {
        shiftKey = true;
    } else if (codelist[0] == 3) {
        shiftKey = true;
        shiftMod = true;
    } 
    ...
    key = new SpiceMsgcKeyUp();
    key.code = 0x80 | codelist[1];
    msg.build_msg(SPICE_MSGC_INPUTS_KEY_UP, key);
    ctx.sc.inputs.send_msg(msg);

    if (shiftMod && !shiftKey) {
        key = new SpiceMsgcKeyUp();
        key.code = 0x80 | KEY_ShiftL;
        msg.build_msg(SPICE_MSGC_INPUTS_KEY_UP, key);
        ctx.sc.inputs.send_msg(msg);
...
function handle_compositionend(e)
{
    console.log("compositionend: " + e.data);
    var codelist = get_codelist_from_tilde_char(e.data);
    if (codelist != undefined) {
        send_char_tilde_key(this, e.charCode, codelist);
    }
}
